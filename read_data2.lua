...
b=a.open("test")
b:loadxlsx("/media/romanp/DATASHURPRO/data/Gesamt.xlsx")
t=b:getsheets()
for z,n in pairs(t)
 do
  print (z,n)
...
print(string.format("insert into auftrag2 values (%d,%d,'%s','%s',%f,'%s','%s');",val1,val5,PostgresEscape(tech),PostgresEscape(art),val2,os.date('%Y-%m-%d',val3),PostgresEscape(val4)))
con:execute(string.format("insert into auftrag2 values (%d,%d,'%s','%s',%f,'%s','%s','%s');",val1,val5,PostgresEscape(tech),PostgresEscape(art),val2,os.date('%Y-%m-%d',val3),PostgresEscape(val4),lltype))
--val=b:lgetnum(0,8)
